...
public class VideoPlayer extends CordovaPlugin implements OnCompletionListener, OnPreparedListener, OnErrorListener, OnDismissListener {
  ...
  protected void openVideoDialog(String path, JSONObject options) {
    Log.v(LOG_TAG, options.toString());
    Log.v(LOG_TAG, cordova.getActivity().getResources().getDisplayMetrics().heightPixels + "");
    Log.v(LOG_TAG, cordova.getActivity().getResources().getDisplayMetrics().widthPixels + "");
    Log.v(LOG_TAG, cordova.getActivity().getResources().getDisplayMetrics().density + "");
   ...
   /* main.setHorizontalGravity(Gravity.CENTER_HORIZONTAL);
    ...
    try {
      ...
      int x = (int) (-(width / 2) + left + (videoWidth / 2));
      int y = (int) (-(height / 2) + top + (videoHeight / 2));
      lp.x = left > (width / 2) ? -x : x;
      lp.y = top > (width / 2) ? -y : y;
